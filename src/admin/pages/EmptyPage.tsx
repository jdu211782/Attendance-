...
function getEmployeeStatus(checkInTime: Date | null, checkOutTime: Date | null): 'Present' | 'Absent' | 'Left' {
    if (!checkInTime) {
        return 'Absent';
    }
    if (checkOutTime) {
...
function EmptyPage() {
    ...
    const [filter, setFilter] = useState<'all' | 'present' | 'absent' | 'left'>('all');
    ...
    const filteredEmployees = employees.filter(employee => {
        ...
        if (filter === 'left' && employee.status !== 'Left') return false;
    ...
    const getStatusColor = (status: 'Present' | 'Absent' | 'Left') => {
        if (status === 'Present') return 'green';
        if (status === 'Left') return 'orange';
        return 'red';
    };
...
<TableBody>
    {paginatedEmployees.map((employee, index) => (
        ...
        >
            ...
            <TableCell style={{ color: employee.status === 'Present' ? 'green' : employee.status === 'Left' ? 'orange' : 'red' }}>
            ...
            <Tooltip title={`Check-in: ${employee.checkInTime ? employee.checkInTime.toLocaleString() : '-'}\nCheck-out: ${employee.checkOutTime ? employee.checkOutTime.toLocaleString() : '-'}`}> 
                <TableCell>
                    <span style={{ color: employee.status === 'Present' ? 'green' : employee.status === 'Left' ? 'orange' : 'red' , padding: '5px', borderRadius: '5px', backgroundColor: employee.status === 'Present' ? 'rgb(179, 239, 183)' : employee.status === 'Left' ? 'rgba(255, 248, 231, 1)' : 'rgba(255, 229, 238, 1)' }}>
                        {employee.status}
                    </span>
                </TableCell>
            </Tooltip>
